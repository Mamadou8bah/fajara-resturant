...
/** Plain-language labels for staff who are not technical. */
...
/** Permissions that cannot be turned off for Owner (prevents lock-out). */
...
export function sanitizeRolePermissions(
  ...
  for (const role of Object.keys(base) as Role[]) {
    ...
    // Everyone keeps credentials.own so they can still change PIN.
